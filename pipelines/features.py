...
def build_daily_feature_table(snapshots: pd.DataFrame) -> pd.DataFrame:
    ...
    pivot["base_deposit_apy"] = pivot[dep_cols].mean(axis=1) if dep_cols else 0.0
    pivot["carry_return_daily"] = pivot.get("funding_rate_daily_drift", pd.Series(0, index=pivot.index)).fillna(0.0) - 0.0001
    ...
    if px_col:
        pivot["realized_vol_7d"] = pivot[px_col].pct_change().rolling(7).std().fillna(0.0)
    else:
        pivot["realized_vol_7d"] = 0.0

    pivot["volatility_score"] = pivot["realized_vol_7d"]
    raw = (
        0.6 * pivot["carry_return_daily"].fillna(0.0)
        + 0.3 * pivot["liquidity_score"].fillna(0.0)
        - 0.1 * pivot["volatility_score"].fillna(0.0)
    )
