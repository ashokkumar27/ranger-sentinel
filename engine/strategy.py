...
class PolicyConfig:
    ...
    risk_on_score: float = 0.70
    defensive_score: float = 0.45

def normalize(base_yield: float, carry: float, reserve: float):
    total = base_yield + carry + reserve
    return {"base_yield": base_yield / total, "carry": carry / total, "reserve": reserve / total}
...
def target_weights(signal: dict, cfg: PolicyConfig | None = None):
    ...
    if score >= cfg.risk_on_score:
        weights = normalize(0.62, min(cfg.max_carry_weight, 0.28), 0.10)
    elif score <= cfg.defensive_score:
        weights = normalize(0.80, 0.08, 0.12)
    else:
        weights = normalize(cfg.base_weight_neutral, cfg.carry_weight_neutral, cfg.reserve_weight_neutral)
